...
public class AddMoneyCommand extends CommandBase {
    ...
    public AddMoneyCommand(Economy main) {
        super("addmoney", "Выдать деньги игроку", main);
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        if (isCorrect(args[1])) {
            economyManager.addMoney(target, parse(args[1]));
            sender.sendMessage("§aВы добавили " + parse(args[1]) + economyManager.getMonetaryUnit() + " игроку " + target.getName() + ".");
        } else {
